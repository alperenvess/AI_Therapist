...
def get_all_suggestions():
    partial_message = request.json.get('message', '')

    with open('intents.json') as json_file:
        intents = json.load(json_file)

    # Get all unique patterns from intents as suggestions
    all_patterns = set()
    for intent in intents['intents']:
        all_patterns.update(intent['patterns'])

    # Filter suggestions based on the user's partial message
    suggestions = [pattern for pattern in all_patterns if pattern.lower().startswith(partial_message.lower())]
